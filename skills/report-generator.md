...
- `SCORE_PERCENT`: 评分百分比 (整数)
- `RATING`: 评级文字
- `RATING_COLOR`: 评级颜色 class
- `ONE_LINER`: 一句话结论
...
- **文本变量** → 直接替换
- **HTML 内容变量**（`QUALITY_SCREEN_CONTENT`, `MOAT_ANALYSIS_CONTENT`, `SAFETY_MARGIN_CONTENT`, `PRICE_RANGE_SECTION`）→ 替换为编排器传入的 HTML 片段。如果编排器未提供某个内容（如价格区间不适用），替换为空字符串 `""`
